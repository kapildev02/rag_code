...
async def create_organization_user(
    ...
    try:
        organization_user = await organization_user_services.create_organization_user(
            organization_user, auth_data.   user_id
        )
        return {
...
async def get_organization_users(auth_data: AuthData = Depends(get_current_user)):
    try:
...
async def update_organization_user(
    ...
    try:
        organization_user = await organization_user_services.update_organization_user(
            organization_user_id, organization_user, auth_data.user_id
        )
        return {
...
async def delete_organization_user(
    organization_user_id: str, auth_data: AuthData = Depends(get_current_user)
):
    try:
        organization_user = await organization_user_services.delete_organization_user(
            organization_user_id, auth_data.user_id
        )
        return {
